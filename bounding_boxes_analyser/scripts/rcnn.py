...
#Ce noeud utilise une version entrainee d un reseau RCNN
...
def callback(image,pub):
    msg = BoundingBoxes()
    msg.bounding_boxes = main(image)
    pub.publish(msg)

def rcnn():
    rospy.init_node('rcnn', anonymous=True)
    pub = rospy.Publisher('/bounding_boxes_analyser/rcnn', BoundingBoxes, queue_size=10)
    # Publie a la reception de chaque message
    rospy.Subscriber('/zed/rgb/image_raw_color', Image, lambda x: callback(x,pub))
